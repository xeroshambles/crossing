...
class ReservationManager:
    ...
    def __init__(self, junction, cellsPerSide, matrixTrajectories, securitySecs):
        ...
        shape = traci.junction.getShape(self.junction.getID())
        self.stop = []
        self.stopCoordinates(shape)  # lista che indica di quanto si distanzia lo stop dal centro
        # dell'incrocio [dx, sotto, sx, sopra]

        self.centerJunctionId = traci.junction.getPosition(self.junction.getID())  # coordinate (x,y) del centro di un
        # incrocio
        ...
        self.cellPassageList = []  # salvo in che cella si trova l'auto in passaggio [cross_id][ [auto , cella_X ,
        # cella_Y ], ...]
        self.precedentPassageList = []  # salvo l'ultima situazione di auto in passaggio per rilasciarle all'uscita
        ...
        x_cell_m = abs(self.cellsLimitsX[1] - self.cellsLimitsX[0])
        y_cell_m = abs(self.cellsLimitsY[1] - self.cellsLimitsY[0])
        x_vehicle_m = traci.vehicle.getHeight("idV0")
        y_vehicle_m = traci.vehicle.getLength("idV0")
                    ...
                    if x[0] == y[0]:
                        ...
                        if self.inCrossing(pos):
                            ...
                            if actual_pos_X != y[1] or actual_pos_Y != y[2]:
                                ...
                                self.cellPassageList[self.cellPassageList.index(y)] = [y[0], actual_pos_X,
                                                                                       actual_pos_Y]
                        # se l'auto è uscita dall'incrocio
                            ...
                            if y[1] is not None and y[2] is not None:
                                ...
                                self.cellPassageList.pop(self.cellPassageList.index(y))
    ...
    def restartVehicle(self, vehicle):
        """Faccio avanzare le auto"""

        traci.vehicle.setSpeedMode(vehicle.idVehicle, 30)
        traci.vehicle.setSpeed(vehicle.idVehicle, traci.vehicle.getMaxSpeed(vehicle.idVehicle))
        self.passageList.append([vehicle.idVehicle, traci.vehicle.getRoadID(vehicle.idVehicle),
                                 traci.vehicle.getAngle(vehicle.idVehicle)])
        self.setInCrossingMatrix(vehicle)
