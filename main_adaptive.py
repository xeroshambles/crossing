...
def trainFromCollectedMeasures(intermediate_group_measures, configs):
    ...
    for config in configs:
        ...
        for m in range(0, len(training_comps)):
            ...
            evaluations[str(config)][m] = {}
            temp = {x: [-1 for t in range(0, n_main_steps)] for x in intermediate_measures[m]}
            for p in intermediate_measures[m]:
                for s in range(0, n_main_steps):
                    sum = 0
                    for repeat in intermediate_measures[m][p]:
                            sum += repeat[s]
                    if len(intermediate_measures[m][p]) > 0:
                        mean = sum / len(intermediate_measures[m][p])
                        temp[p][s] = mean
                    else:
                        temp[p][s] = 0
            index = 0
            for s in range(0, n_main_steps):
                ...
                index += 1
            evaluations[str(config)][m] = evaluation
    return evaluations
    ...
    for spawn_config in spawn_configs:
        ...
        for project in projs:
            ...
            for rt in range(0, repeat_train):
                ...
                if project == "adaptive":
                    dir = os.path.join(path, f"adaptive_{titles[rt]}_{spawn_config}_training")
                    tf = open(os.path.join(path, f"{spawn_config}_training_sets.txt"), "a")
                    tf.write(f"{titles[rt]} : {sims_per_main_step[str(numberOfVehicles[i])][rt]}\n")
                    tf.close()
                else:
                    dir = os.path.join(path, f"{project}_{spawn_config}")
                ...
                for i in range(0, diff):
                    ...
                    for m in range(0, len(titles)):
                        if m not in intermediate_group_measures[str(numberOfVehicles[i])].keys():
                            intermediate_group_measures[str(numberOfVehicles[i])][m] = {}
                        intermediate_group_measures[str(numberOfVehicles[i])][m][project] = []
                    ...
                    if train:
                        ...
                        queue = simulate(project=sim, procs=[], queue=queue, train=sims_per_main_step[str(numberOfVehicles[i])][rt], i=i)
